...
dataset_ref_to_datasets = {
    ...
    "bpic2011": ["bpic2011_f%s_exp_prefix_new"%formula for formula in range(1,5)],
    #"bpic2015": ["bpic2015_%s_f2"%(municipality) for municipality in range(1,6)],
    "bpic2015": ["bpic2015_%s_f2_exp_prefix_new"%(municipality) for municipality in range(1,6)],
    "production": ["production"],
    #"production": ["production_exp_prefix_new"],
    "traffic_fines_1": ["traffic_fines_1"],
    #"traffic_fines_1": ["traffic_fines_1_exp_prefix_new"],
    "insurance": ["insurance_activity", "insurance_followup"],
    #"sepsis_cases": ["sepsis_cases_1", "sepsis_cases_2", "sepsis_cases_4"],
    "sepsis_cases": ["sepsis_cases_1_exp_prefix_new", "sepsis_cases_2_exp_prefix_new", "sepsis_cases_4_exp_prefix_new"]
}
...
#datasets = ["bpic2015_2_f2_exp_prefix_new"] #Delete this sentence when conducting real experiments
        ...
        for bucket in set(bucket_assignments_test):
            ...
            if len(relevant_train_cases_bucket) == 0:
                ...
                if len(set(train_y)) < 2:
                    ...
                    #preds_temp = []
                    #test_y_all_temp = test_y_all.copy()
                    #test_all_grouped_temp = dt_test_bucket.groupby(dataset_manager.case_id_col)
                    #for __temp, group_temp in test_all_grouped_temp:
                    #    test_y_all_temp.extend(dataset_manager.get_label_numeric(group_temp))
                    #pipeline.fit(dt_train_bucket, train_y, cls__early_stopping_rounds=15, cls__eval_set=[[pipeline.named_steps["encoder"].transform(dt_test_bucket), test_y_all_temp]])

                    offline_time_fit += time.time() - start_offline_time_fit

                    feat_imp = pd.DataFrame(columns=["feature_name","feature_imp"])
                    feat_imp['feature_name'] = pipeline.steps[0][1].get_feature_names()
                    feat_imp['feature_imp'] = pipeline.steps[1][1].feature_importances_
                    feat_imp.to_csv("feature_importances_" + dataset_name + "_" + cls_method + "_" + method_name + ".csv",index=False)
